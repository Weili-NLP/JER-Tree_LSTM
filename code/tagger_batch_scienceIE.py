...
from utils import get_name, models_path, evaluate, test, eval_script#, eval_temp
...
if opts.dev:
    ...
    dev_score = evaluate(parameters, f_eval, dev_sentences, dev_data, id_to_tag, eval_temp, -1, 'valid')
...
for line in open(opts.test):
    ...
    try:
        ...
        outfn = line.rstrip().split('\t')[1] + 'IE'
        test_score = test(parameters, f_eval, test_sentences,
                          test_data, id_to_tag, dico_tags, eval_temp, 0, outfn, f_eval_softmax)
    except:
        print fn
