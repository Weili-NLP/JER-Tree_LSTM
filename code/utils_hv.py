...
def test(parameters, f_eval_hv, raw_sentences, parsed_sentences, tag_to_id, fx, fy):
    """
    Evaluate current model using CoNLL script.
    """
    n_tags = len(tag_to_id)
    print fx,fy
    with open(fx, 'wb') as f1, open(fy, 'wb') as f2:
        for raw_sentence, data in zip(raw_sentences, parsed_sentences):
            input = create_input(data, parameters, False)
            hidden_vec = f_eval_hv(*input)
            for i, word in enumerate(raw_sentence):
		new_line = "\t".join([str(v) for v in hidden_vec[i].tolist()])
		f1.write(new_line)
                f1.write('\n')
		yid = tag_to_id[word[1]]
                one_hot_y = np.zeros((n_tags),dtype=np.int32)
		one_hot_y[yid] = 1
                new_line = "\t".join([str(v) for v in one_hot_y.tolist()])
		f2.write(new_line)
                f2.write('\n')
