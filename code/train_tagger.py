...
if opts.reload == None:
    if opts.train_true:
        ...
        dico_Vs, V_to_id, id_to_V = dep_verb_mapping(train_sentences+ train_true_sentences)
        dico_Ns, N_to_id, id_to_N = dep_noun_mapping(train_sentences + train_true_sentences)
    else:
        dico_chars, char_to_id, id_to_char = char_mapping(train_sentences)
        dico_tags, tag_to_id, id_to_tag = tag_mapping(train_sentences)
        dico_POSs, POS_to_id, id_to_POS = pos_mapping(train_sentences)
        dico_Vs, V_to_id, id_to_V = dep_verb_mapping(train_sentences)
        dico_Ns, N_to_id, id_to_N = dep_noun_mapping(train_sentences)
dico_Vs, V_to_id, id_to_V = dep_verb_mapping(train_sentences)
dico_Ns, N_to_id, id_to_N = dep_noun_mapping(train_sentences)
dico_tags, tag_to_id, id_to_tag = tag_mapping(train_sentences)
        
# Index data
# fid = open('/homes/luanyi/pubanal/project/code/char_lstm/models/SGD_tie_lstm_25_25_2016111023_2487/vocab.dct')
# char_to_id = cPickle.load(fid)
# fid.close()
# id_to_char = {}

# for char in char_to_id:
#     id_to_char[char_to_id[char]] = char
if opts.reload != None:
    word_to_id, char_to_id, _, POS_to_id = [
...
train_data = prepare_dataset(
    train_sentences, word_to_id, char_to_id, tag_to_id, POS_to_id, V_to_id, N_to_id, lower
)
if opts.train_true:
    train_true_data = prepare_dataset(
        train_true_sentences, word_to_id, char_to_id, tag_to_id, POS_to_id, V_to_id, N_to_id, lower
)

dev_data = prepare_dataset(
    dev_sentences, word_to_id, char_to_id, tag_to_id, POS_to_id, V_to_id, N_to_id, lower
)
test_data = prepare_dataset(
    test_sentences, word_to_id, char_to_id, tag_to_id, POS_to_id, V_to_id, N_to_id, lower
)
...
model.save_mappings(id_to_word, id_to_char, id_to_tag, id_to_POS, id_to_N, id_to_V)
# pdb.set_trace()
# Build the model
...
freq_eval = len(train_data)/2  # evaluate on dev every freq_eval steps
best_dev = -np.inf
best_test = -np.inf
count = 0
n_valid=0
...
dev_score = evaluate(parameters, f_eval, dev_sentences,
                     dev_data, id_to_tag, eval_temp, n_valid)
...
for epoch in xrange(n_epochs):
    ...
    for i, index in enumerate(np.random.permutation(len(train_data))):
        ...
        [new_cost,a,b] = f_train(*input)
        if opts.train_true:
            if n_true >= len(train_true_data):
                n_true = 0
            input = create_input(train_true_data[n_true], parameters, True, singletons)
            [new_cost,a,b] = f_train(*input)
            n_true += 1
        ...
        if count % freq_eval == 0:
            ...
            test_score = evaluate(parameters, f_eval, test_sentences,
                                  test_data, id_to_tag, eval_temp, n_valid)
