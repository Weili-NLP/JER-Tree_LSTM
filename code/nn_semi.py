...
def forward(observations, transitions, viterbi=False,
    ...
    if return_alpha:
        ...
        # scores = alpha[T.arange(s_len+2), ]
        sequence = T.concatenate([sequence[::-1], [T.argmax(alpha[0][-1])]])
        return sequence, observations, T.max(alpha[0][-1])
    else:
        if viterbi:
...
def forward_partial(observations, transitions, padded_tag_ids, viterbi=False,
            return_alpha=False, return_best_sequence=False):
    """
    Takes as input:
        - observations, sequence of shape (n_steps, n_classes)
    ...
    def recurrence(obs, tagtm1, tagt, previous, transitions):
        previous = previous.dimshuffle(0, 'x') 
        tagtm1 = tagtm1.dimshuffle(0, 'x')
        previous = tagtm1 * previous
        obs = obs.dimshuffle('x', 0)
        tagt = tagt.dimshuffle('x', 0)
        obs = tagt * obs
        transitions = transitions * tagt * tagtm1
        if viterbi:
            scores = previous + obs + transitions
            out = scores.max(axis=0)
            if return_best_sequence:
    ...
    alpha, _ = theano.scan(
        fn=recurrence,
        outputs_info=(initial, None) if return_best_sequence else initial,
        sequences=[dict(input=observations[1:],taps=[0]), dict(input=padded_tag_ids,taps=[-1,-0])],
        non_sequences=transitions
    )
